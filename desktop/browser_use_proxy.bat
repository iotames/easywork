...
if defined paths[%count%] (
    ...
    if exist "!paths[%count%]!" (
        ...
        REM 启动浏览器并传递参数
        start "StartBrowserUseProxy" "!paths[%count%]!" --log-level=1 --window-size="1366,768" --user-data-dir="!USER_DATA_DIR!" --proxy-server=%PROXY_SERVER_URL% --new-window %TARGET_URL%
        goto end
    ) else (
