...
| ----- | --------- | ----------- | ------- | ------- | ------- |
| 1 | 跳转到Oauth2服务端的指定地址，为了获取code参数 | GET |  Oauth2服务端提供  |包含回调地址，址如：redirect_uri|回调地址为关键参数，一般为redirect_uri或redirect|
| 2  | 获取Oauth2服务端传回的code参数  | GET  |  /oauth2/client/callback  | ?code=xxx|从上一步的Oauth2服务端跳转回来Oauth2客户端，获取GET参数code的值|
...
`目的`: 接收从Oauth2服务端跳转过来时携带的GET参数code
