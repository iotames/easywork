import{D as e,F as t,O as n,f as r}from"./app-CviA4zwQ.js";var i=JSON.parse(`{"path":"/linux/systemd/","title":"Systemd与Systemctl","lang":"zh-CN","frontmatter":{"title":"Systemd与Systemctl","description":"systemd 初始化系统与 systemctl 服务管理命令","tags":["Linux","systemd","systemctl"],"order":2},"git":{"updatedTime":1787587392000,"contributors":[{"name":"Hankin","username":"Hankin","email":"hankin@catmes.com","commits":3,"url":"https://github.com/Hankin"},{"name":"吴汉青","username":"","email":"554553400@qq.com","commits":3}],"changelog":[{"hash":"3497da8069a6dc7827dab9b0489083546c3563dd","time":1787587392000,"email":"554553400@qq.com","author":"吴汉青","message":"update codex"},{"hash":"4b2b07034b089c869029b6e26948bf7d06d8bb65","time":1786520319000,"email":"554553400@qq.com","author":"吴汉青","message":"docs: 完善邮箱迁移方案，新增 Codex Agent 说明，补齐各 README frontmatter 并重排顶层目录顺序"},{"hash":"15ed9225f8399059e9d54a0484f769a882118676","time":1779163154000,"email":"554553400@qq.com","author":"吴汉青","message":"UPDATE systemd/README.md"},{"hash":"243b9d33d875cfd3cf0ebfaebf7d957020edd0b1","time":1759204370000,"email":"554553400@qq.com","author":"Hankin","message":"UPDATE systemd"},{"hash":"028e0006fb19cf5c192d554d2978465965f11e4a","time":1759131855000,"email":"554553400@qq.com","author":"Hankin","message":"UPDATE systemd.md: journalctl --vacuum-size=500M"},{"hash":"e8519aa41da5fe2f3877981a6c3e47115b07b22c","time":1752418311000,"email":"hankin@catmes.com","author":"Hankin","message":"ADD linux/systemd.md. UPDATE promtail"}]},"filePathRelative":"linux/systemd/README.md"}`),a={name:`README.md`};function o(r,i,a,o,s,c){return t(),e(`div`,null,[...i[0]||=[n(`<h2 id="systemctl和systemd简介" tabindex="-1"><a class="header-anchor" href="#systemctl和systemd简介"><span>systemctl和systemd简介</span></a></h2><ul><li>https://github.com/systemd/systemd</li></ul><p><code>systemd</code> 即： <code>system daemon</code>。源于Unix中的一个惯例：在Unix中常以 <code>d</code> 作为系统守护进程（英语：daemon）的后缀标识。</p><p><code>systemctl</code> 是 <code>systemd</code> 的一个命令行工具，用于管理 <code>systemd</code> 服务。<code>service</code> 则是Linux系统的另一种服务管理方式。</p><h2 id="systemctl-和-service" tabindex="-1"><a class="header-anchor" href="#systemctl-和-service"><span>systemctl 和 service</span></a></h2><ol><li>所属体系: <code>systemctl</code> 属于 <code>systemd</code> init系统，<code>service</code> 属于早期的 <code>SysVinit</code> init系统</li><li>控制范围: <code>service</code> 命令一般只能 启动、停止、重启和查看服务，<code>systemctl</code> 命令则更广。</li><li>开机流程: 开机系统初始化时，<code>SysVinit</code> 让多个任务依次启动，<code>systemd</code> 则是梳理依赖关系，并行启动所有任务。</li><li>应用目录: <code>service</code> 主要在 <code>/etc/init.d</code>，而 <code>systemctl</code> 在 <code>/etc/systemd/system</code> 等目录。</li></ol><p>service和systemctl命令对比：</p><div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre><code class="language-bash"><span class="line"><span class="token comment"># 启动，停止，重启(先stop后再start)，查看状态</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="systemd-配置文件目录" tabindex="-1"><a class="header-anchor" href="#systemd-配置文件目录"><span>systemd 配置文件目录</span></a></h2><ol><li>自定义服务（优先级最高）： <code>/etc/systemd/system/</code></li><li>系统服务(不推荐修改)：<code>/usr/lib/systemd/system/</code></li></ol><ul><li><p><code>/etc/systemd/system/</code>: 管理员 <code>自定义</code> 和服务覆盖配置的核心目录，优先级最高。部分为指向低优先级服务文件的软链接，用于定义开机启动。</p></li><li><p><code>/usr/lib/systemd/system/</code>: 存放系统服务 <code>默认配置</code>，不应直接修改它。类似 <code>/etc/init.d/</code>。</p></li><li><p><code>/lib/systemd/system/</code>: 提供指向 <code>/usr/lib/systemd/system/</code> 的符号链接，目的是兼容旧路径。</p></li><li><p><code>/run/systemd/system/</code>：保存系统运行产生的动态配置，优先级比 <code>/usr/lib/systemd/system/</code> 高。</p></li><li><p><code>/usr/lib/systemd/user/</code>: 存放用户级脚本的默认配置，登录后才可运行</p></li><li><p><code>~/.config/systemd/user/</code>:仅当前用户可用。</p></li></ul><h2 id="systemd自定义服务示例" tabindex="-1"><a class="header-anchor" href="#systemd自定义服务示例"><span>systemd自定义服务示例</span></a></h2><p>例：<code>vim /etc/systemd/system/qddns.service</code>:</p><div class="language-conf line-numbers-mode" data-highlighter="prismjs" data-ext="conf"><pre><code class="language-conf"><span class="line">[Unit]</span>
...
<span class="line">StandardOutput=file:/root/qddns/output.log</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><ol start="2"><li>journalctl 命令</li></ol><div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre><code class="language-bash"><span class="line"><span class="token comment"># 查看某服务的日志</span></span>
<span class="line">journalctl <span class="token parameter variable">-u</span> nginx.service</span>
<span class="line"></span>
<span class="line"><span class="token comment"># 查看日志已占用的空间</span></span>
<span class="line">journalctl --disk-usage</span>
<span class="line"></span>
<span class="line"><span class="token comment"># 设置日志最大占用空间: 500M, 2G</span></span>
<span class="line">journalctl --vacuum-size<span class="token operator">=</span>500M</span>
<span class="line"></span>
<span class="line"><span class="token comment"># 设置日志最大保存时间: 10d, 1years</span></span>
<span class="line">journalctl --vacuum-time<span class="token operator">=</span>30d</span>
